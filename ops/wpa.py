...
def compute_wpa(img_emb1, img_emb2, labels):

    with torch.cuda.amp.autocast(enabled=False):
        txt_pad = torch.zeros(img_emb1.size(0), img_emb1.size(1), dtype=torch.bool, device=img_emb1.device)
        img_pad = torch.zeros(img_emb2.size(0), img_emb2.size(1), dtype=torch.bool, device=img_emb2.device)

        cost = cost_matrix_cosine(img_emb1.float(), img_emb2.float())
        joint_pad = txt_pad.unsqueeze(-1) | img_pad.unsqueeze(-2)
        cost.masked_fill_(joint_pad, 0)
    ...
    dist_pos = distance.masked_select(labels == 1)
    dist_neg = distance.masked_select(labels == 0)
